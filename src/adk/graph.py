"""ADK Agent adapter for ADAM Orchestrator pipeline."""
...
class AdamAgent(Agent):
    """ADAM agent exposing orchestrator pipeline to ADK web UI."""
    
    name: str = Field(default="adam", description="Agent name")
    model: str = Field(default="gemini-2.0-flash", description="Default model")
    orchestrator: Any = Field(default=None, description="Orchestrator instance")
    ...
    def chat(self, message: str) -> str:
        """
        Free-form chat: interpret keywords and call ingest/decide/index/ask.
